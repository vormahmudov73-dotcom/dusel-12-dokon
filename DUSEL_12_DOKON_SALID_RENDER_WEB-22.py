...
def show_section_manage(chat_id, brand):
    ...
    buttons=[("📁 "+g, "sectiongroupadmin|"+brand+"|"+g) for g in groups]
    for sec in sections:
        if sec not in grouped_children: buttons.append(("📂 "+sec, "sectionaction|"+brand+"|"+sec))
    if len(sections)>=2:
        buttons.append(("🔗 2–3 bo‘limni birlashtirish", "sectionmerge|"+brand))
    buttons.append(("⬅️ Brend", "brandmanage|"+brand))
    send_message(chat_id, "📂 <b>"+brand+" BO‘LIMLARI</b>\n\nBo‘limni tanlang yoki 2–3 ta bo‘limni bitta guruhga birlashtiring:", keyboard(buttons))
...
def start_section_merge(chat_id, brand):
    ...
    send_message(chat_id,"🔗 <b>Birlashtiriladigan bo‘limlarni tanlang</b>\n\n2 yoki 3 ta bo‘limni tanlang. Tanlangan bo‘limga yana bosib bekor qilishingiz mumkin.",keyboard(buttons))
...
def search_products(chat_id, query):
    ...
    if len(q) < 2:
        send_message(chat_id, "❌ Kamida 2 ta belgi kiriting.")
        return
        ...
        for section, items in sections.items():
            if isinstance(items, dict):
                for color, color_items in items.items():
                    for index, (product, price) in enumerate(color_items):
                        hay = (str(product) + " " + str(color) + " " + str(section) + " " + str(brand)).casefold()
                        if q in hay:
                            results.append((brand, section, color, index, product, price))
            else:
                for index, (product, price) in enumerate(items):
                    hay = (str(product) + " " + str(section) + " " + str(brand)).casefold()
                    if q in hay:
                        results.append((brand, section, None, index, product, price))

    if not results:
        send_message(chat_id, "🔎 <b>Natija topilmadi.</b>\n\nBoshqa nom yoki kod bilan qidirib ko‘ring.", search_menu())
        return

    buttons = []
    for brand, section, color, index, product, price in results[:50]:
        # Natijada mahsulotning qayerdaligi ham ko‘rinadi.
        location = brand + " / " + section
        if color:
            location += " / " + color
            cb = "product|" + brand + "|" + section + "|" + color + "|" + str(index)
        else:
            cb = "product|" + brand + "|" + section + "|" + str(index)
        label = product + " — $" + format(price)
        if len(label) > 45:
            label = label[:45] + "…"
        # Telegram tugmasida joylashuvni qisqa ko‘rsatamiz.
        short_loc = location if len(location) <= 28 else location[:28] + "…"
        label = "📦 " + label + "\n📂 " + short_loc
        if len(cb.encode("utf-8")) > 64:
            continue
        buttons.append((label, cb))

    text = "🔎 <b>Qidiruv natijalari:</b> " + str(len(results)) + " ta\n\n"
    if len(results) > 50:
        text += "Birinchi 50 ta natija ko‘rsatildi.\n\n"
    text += "Mahsulotni tanlang:"
    send_message(chat_id, text, keyboard(buttons + [("🔎 Yana qidirish", "search_start"), ("⬅️ Asosiy menyu", "back_menu")]))
...
def show_sections(chat_id, brand):
    ...
    for section in sections:
        if section not in grouped_children:
...
def show_section_group(chat_id, brand, group):
    ...
    buttons=[(c, "section|"+brand+"|"+c) for c in children]
    buttons.append(("⬅️ "+brand, "brand|"+brand))
    send_message(chat_id, "📁 <b>"+group+"</b>\n\nIchki bo‘limni tanlang:", keyboard(buttons))
...
def handle_text(chat_id, text):
    ...
    if step == "admin_group_rename" and chat_id == ADMIN_ID:
    ...
    if step == "admin_section_merge_name" and chat_id == ADMIN_ID:
        group=text.strip(); brand=(data or {}).get("brand",""); selected=(data or {}).get("selected",[])
        if len(group)<2 or len(selected)<2 or len(selected)>3:
...
def handle_callback(callback):
    ...
    try:
        ...
        if data.startswith("sectiongroupadmin|") and chat_id == ADMIN_ID:
            parts=data.split("|",2); show_section_group_admin(chat_id,parts[1],parts[2]); return
        if data.startswith("sectionmerge|") and chat_id == ADMIN_ID:
            start_section_merge(chat_id,data.split("|",1)[1]); return
        if data.startswith("mergesel|") and chat_id == ADMIN_ID:
        ...
        if data.startswith("mergefinish|") and chat_id == ADMIN_ID:
        ...
        if data.startswith("groupungroup|") and chat_id == ADMIN_ID:
        ...
        if data.startswith("grouprename|") and chat_id == ADMIN_ID:
        ...
        if data == "search_start":
            ...
            send_message(chat_id, "🔎 <b>Mahsulot qidirish</b>\n\nMahsulot nomi yoki kodini yozing:")
            return
...
def seed_catalog_fixes():
    ...
    # DUSEL bo‘limlarini katta guruhlarga yig‘amiz. Mahsulotlar o‘chirilmaydi;
    # faqat menyuda guruh ichida ko‘rinadi. Har ishga tushganda qayta tiklanadi.
    cur.execute("DELETE FROM section_groups WHERE brand=?", ("DUSEL",))

    existing = {r[0] for r in cur.execute(
        "SELECT section FROM dynamic_sections WHERE brand=?", ("DUSEL",)
    ).fetchall()}
    ...
    # AKRIL-PANEL + AKRIL VA PANEL
    add_group("AKRIL VA PANEL", sorted(
        [x for x in existing if x.startswith("AKRIL-PANEL / ") or x.startswith("AKRIL VA PANEL / ")]
    ))

    # Barcha proyektor bo‘limlari
    add_group("PROYEKTORLAR", sorted(
        [x for x in existing if x.startswith("PRAJECKTOC-RKU / ")]
    ))

    # Aksessuarlar ichki bo‘limlari
    add_group("AKSESSUARLAR", sorted(
        [x for x in existing if x == "AKSESSUARLAR" or x.startswith("AKSESSUARLAR / ")]
    ))

    # Slim LED ichki bo‘limlari
    add_group("SLIM LED", sorted(
        [x for x in existing if x.startswith("SLIM LED T5 / ")]
    ))
