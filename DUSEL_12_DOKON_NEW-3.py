...
def admin_section_rows(action):
    ...
    for brand, sections in CATALOG.items():
        for section in sections:
            rows.append((f"{brand} / {section}", f"{action}:{brand}:{section}"))
...
def admin_handle_text(chat_id, text):
    ...
    try:
        if state[0] == "add_brand":
            ...
            CATALOG[name]={}; save_state(); rebuild_ids(); ADMIN_STATE.pop(chat_id,None)
        ...
        if state[0] == "rename_brand":
            ...
            CATALOG[new]=CATALOG.pop(old); save_state(); rebuild_ids(); ADMIN_STATE.pop(chat_id,None)
        ...
        if state[0] == "add_section":
            ...
            CATALOG[brand][name]=[]; save_state(); rebuild_ids(); ADMIN_STATE.pop(chat_id,None)
        ...
        if state[0] == "rename_section":
            ...
            CATALOG[brand][new]=CATALOG[brand].pop(old); save_state(); rebuild_ids(); ADMIN_STATE.pop(chat_id,None)
...
def callback(update):
    ...
    if data == "adm_rename_section":
        send(chat_id,"✏️ O'zgartiriladigan bo'limni tanlang:",kb(admin_section_rows("adm_rsec"))); return
    if data.startswith("adm_rsec:"):
        _,brand,section=data.split(":",2); admin_prompt(chat_id,("rename_section",brand,section),"✏️ Yangi bo'lim nomini yozing:"); return
    if data == "adm_del_section":
        send(chat_id,"🗑 O'chiriladigan bo'limni tanlang:",kb(admin_section_rows("adm_dsec"))); return
    if data.startswith("adm_dsec:"):
        _,brand,section=data.split(":",2)
        if brand in CATALOG and section in CATALOG[brand]:
            ...
            save_state(); rebuild_ids(); send(chat_id,"✅ Bo'lim o'chirildi.",admin_menu())
        return
    if data == "adm_add_product":
        send(chat_id,"➕ Mahsulot qaysi bo'limga qo'shiladi?",kb([(f"{b} / {s}",f"adm_aprod:{b}:{s}") for b,ss in CATALOG.items() for s in ss] + [("⬅️ Admin","admin")])); return
    if data.startswith("adm_aprod:"):
        _,brand,section=data.split(":",2); admin_prompt(chat_id,("add_product",brand,section),"📦 Format: Mahsulot nomi | narx\nMasalan: Lamp 10W | 1.25"); return
    if data == "adm_price":
