...
def admin_menu():
    # Asosiy admin menyusi reply-keyboard ko\'rinishida.
    return reply_kb([
        ["📦 Mahsulotlar", "📋 Spiskalar"],
        ["👥 Mijozlar", "📦 Buyurtmalar"],
        ["🎁 Chegirmalar", "📢 Xabar yuborish"],
        ["➕ Bo\'lim qo\'shish", "➕ Mahsulot qo\'shish"],
        ["➕ Brend qo\'shish", "🛠 Brend/bo\'lim boshqaruvi"],
        ["💵 Price boshqaruvi", "📞 Admin bilan bog\'lanish"],
        ["🖼 Rasmlar", "📊 Excel"],
        ["⬅️ Asosiy menyu"]
    ])
...
# Telegram juda katta inline keyboardni qabul qilmasligi mumkin.
# Admin ro'yxatlari sahifalarga bo'linadi.
ADMIN_PAGE_SIZE = 12

def _admin_sections_flat():
    return [(b, s, sid) for (b, s), sid in SECTION_IDS.items()]

def _admin_products_flat():
    out = []
    for brand, sections in CATALOG.items():
        for section, items in sections.items():
            for idx, (product, price) in enumerate(items):
                pid = PRODUCT_IDS.get((brand, section, idx))
                if pid is not None:
                    out.append((brand, section, idx, product, price, pid))
    return out

def admin_paginated_rows(kind, action, page=0):
    page = max(0, int(page))
    rows = []
    if kind == "sections":
        items = _admin_sections_flat()
        total = len(items)
        start = page * ADMIN_PAGE_SIZE
        for brand, section, sid in items[start:start + ADMIN_PAGE_SIZE]:
            rows.append((f"📂 {brand} / {section}", f"{action}:{sid}"))
    elif kind == "products":
        items = _admin_products_flat()
        total = len(items)
        start = page * ADMIN_PAGE_SIZE
        for brand, section, idx, product, price, pid in items[start:start + ADMIN_PAGE_SIZE]:
            rows.append((f"📦 {brand} | {product} — ${price:g}", f"{action}:{pid}"))
    elif kind == "add_products":
        items = _admin_sections_flat()
        total = len(items)
        start = page * ADMIN_PAGE_SIZE
        for brand, section, sid in items[start:start + ADMIN_PAGE_SIZE]:
    ...
    nav = []
    if page > 0:
        nav.append(("⬅️ Oldingi", f"adm_page:{kind}:{action}:{page-1}"))
    if page + 1 < pages:
        nav.append(("Keyingi ➡️", f"adm_page:{kind}:{action}:{page+1}"))
    if nav:
        rows.append(tuple(nav))
    rows.append(("⬅️ Admin", "admin"))
    return rows, total

def send_admin_page(chat_id, kind, action, page=0):
    rows, total = admin_paginated_rows(kind, action, page)
    title = {
        "sections": "📂 <b>Bo'limlar</b>",
        "products": "📦 <b>Mahsulotlar</b>",
        "add_products": "➕ <b>Mahsulot qo'shish</b>"
    }.get(kind, "⚙️ <b>Admin</b>")
    page_no = int(page) + 1
    pages = max(1, (total + ADMIN_PAGE_SIZE - 1) // ADMIN_PAGE_SIZE)
    send(chat_id, f"{title}\n\nSahifa: <b>{page_no}/{pages}</b>\nJami: <b>{total}</b> ta", kb(rows))
...
def callback(update):
    ...
    if data == "adm_brands":
        send(chat_id, "🏷 <b>Brendlar</b>\n\nBrendni tanlang:", kb(admin_brand_rows("adm_rbrand"))); return
    if data == "adm_sections":
        send_admin_page(chat_id, "sections", "adm_secview", 0); return
    if data.startswith("adm_page:"):
        try:
            _, kind, action, page_s = data.split(":", 3)
            page = int(page_s)
            send_admin_page(chat_id, kind, action, page)
        except Exception:
            send(chat_id, "❌ Sahifa ochilmadi.", admin_menu())
        return
    if data.startswith("adm_secview:"):
        try:
        ...
        if item:
            brand,section=item
            count=len(CATALOG.get(brand,{}).get(section,[]))
            send(chat_id, f"📂 <b>{section}</b>\n🏷 Brend: <b>{brand}</b>\n📦 Mahsulotlar: <b>{count} ta</b>", kb([
                ("➕ Mahsulot qo'shish", f"adm_aprodid:{sid}"),
                ("✏️ Nomini o'zgartirish", f"adm_rsecid:{sid}"),
                ("🗑 O'chirish", f"adm_dsecid:{sid}"),
                ("⬅️ Bo'limlar", "adm_sections"),
                ("⬅️ Admin", "admin")
            ]))
        return
    ...
    if data.startswith("adm_dbrand:"):
        ...
        if brand in CATALOG:
            del CATALOG[brand]; save_state(); rebuild_ids(); sync_navigation()
    ...
    if data == "adm_rename_section":
        send_admin_page(chat_id, "sections", "adm_rsecid", 0); return
    ...
    if data == "adm_del_section":
        send_admin_page(chat_id, "sections", "adm_dsecid", 0); return
    ...
    if data == "adm_add_product":
        send_admin_page(chat_id, "add_products", "adm_aprodid", 0); return
    if data.startswith("adm_aprodid:"):
        ...
        return
    if data == "adm_price":
        send_admin_page(chat_id, "products", "adm_pedit", 0); return
    ...
    if data == "adm_del_product":
        send_admin_page(chat_id, "products", "adm_dprod", 0); return
...
def message(update):
    ...
    if is_admin(chat_id) and not ADMIN_STATE.get(chat_id):
        if text in ("📦 Mahsulotlar", "📋 Spiskalar"):
            send(chat_id, admin_products_text(), admin_menu())
            return
        if text == "➕ Mahsulot qo\'shish":
            rows = [(f"{b} / {s}", f"adm_aprodid:{sid}") for (b, s), sid in SECTION_IDS.items()]
            rows.append(("⬅️ Admin", "admin"))
            send(chat_id, "➕ <b>Mahsulot qaysi bo\'limga qo\'shiladi?</b>", kb(rows))
            return
        if text == "➕ Bo\'lim qo\'shish":
            rows = [(b, "adm_absec:" + b) for b in CATALOG]
            rows.append(("⬅️ Admin", "admin"))
            send(chat_id, "➕ <b>Bo\'lim qaysi brendga qo\'shiladi?</b>", kb(rows))
            return
        if text == "➕ Brend qo\'shish":
            admin_prompt(chat_id, ("add_brand",), "🏷 <b>Yangi brend nomini yozing:</b>")
            return
        if text == "🛠 Brend/bo\'lim boshqaruvi":
            send(chat_id, "🛠 <b>Brend / bo\'lim boshqaruvi</b>", admin_management_menu())
            return
        if text == "💵 Price boshqaruvi":
            send(chat_id, "💰 <b>Narxi o\'zgartiriladigan mahsulotni tanlang:</b>", kb(admin_product_rows("adm_pedit")))
            return
        if text == "📞 Admin bilan bog\'lanish":
            admin_prompt(chat_id, ("contact",), "📞 <b>Format:</b> Telegram | Telefon")
            return
        if text == "⬅️ Asosiy menyu":
            send(chat_id, "🏠 <b>Asosiy menyu</b>", main_menu())
            return
        if text == "👥 Mijozlar":
            send(chat_id, "👥 <b>Mijozlar</b>\n\nMijozlar ro\'yxati bu versiyada saqlanmaydi.", admin_menu())
            return
        if text == "📦 Buyurtmalar":
            send(chat_id, "📦 <b>Buyurtmalar</b>\n\nSaqlangan buyurtmalar ro\'yxati bu versiyada mavjud emas.", admin_menu())
            return
        if text == "🎁 Chegirmalar":
            send(chat_id, "🎁 <b>Chegirmalar</b>\n\nChegirma boshqaruvi hali ulanmagan.", admin_menu())
            return
        if text == "📢 Xabar yuborish":
            send(chat_id, "📢 <b>Xabar yuborish</b>\n\nOmmaviy xabar funksiyasi bu versiyada ulanmagan.", admin_menu())
            return
        if text == "🖼 Rasmlar":
            send(chat_id, "🖼 <b>Rasmlar</b>\n\nRasm boshqaruvi bu versiyada hali ulanmagan.", admin_menu())
            return
        if text == "📊 Excel":
            send(chat_id, "📊 <b>Excel</b>\n\nExcel eksporti bu versiyada hali ulanmagan.", admin_menu())
            return
