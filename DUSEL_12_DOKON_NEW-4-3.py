...
ADMIN_ID = int(os.getenv("ADMIN_ID", "8914759604"))
...
def callback(update):
    ...
    if data == "adm_brands":
        send(chat_id, "🏷 <b>Brendlar</b>\n\nBrendni tanlang:", kb([(f"🏷 {b}", f"adm_rbrand:{b}") for b in CATALOG] + [("⬅️ Admin", "admin")])); return
    if data == "adm_sections":
        rows=[]
        for (b,s), sid in SECTION_IDS.items():
            rows.append((f"📂 {b} / {s}", f"adm_secview:{sid}"))
        if not rows:
            rows.append(("➕ Bo'lim qo'shish", "adm_add_section"))
        rows.append(("⬅️ Admin", "admin"))
        send(chat_id, "📂 <b>Bo'limlar</b>\n\nKerakli bo'limni tanlang:", kb(rows)); return
    if data.startswith("adm_secview:"):
        try:
            sid=int(data.split(":",1)[1])
            item=section_by_id(sid)
        except Exception:
            item=None
        if item:
            brand,section=item
